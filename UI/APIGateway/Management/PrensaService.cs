...
namespace APIGateway.Management
{
	public class PrensaService : BaseBalancerService
	{
		...
		public override async Task<APIConfiguration> GetConfigurationRequest()
		{
			...
			var result = await _httpClient.GetAsync("api/Prensa/configuration");
		...
		public override async Task<IEnumerable<PackageItem>> ListPackages()
		{
			...
			var result = await _httpClient.GetAsync("api/Prensa/packages");
		...
		public override async Task<ServiceStatus> GetStatusRequest()
		{
			...
			var result = await _httpClient.GetAsync("api/Prensa/status");
		...
		public override async Task SendStatusRequest(ServiceStatus status)
		{
			...
			var result = await _httpClient.PostAsync($"api/Prensa/status?type={status}", content);
		...
		public override async Task UpdateConfigurationRequest(APIConfiguration item)
		{
			...
			var result = await _httpClient.PutAsync("api/Prensa/configuration", content);
